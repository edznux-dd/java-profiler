...
class DwarfParser {
  private:
    ...
    const char* _ptr;
    // Read window [_section_start, _section_end). The parsed data comes from a
    // memory-mapped ELF image that may be truncated, corrupt, or hostile, so
    // every read funnels through the helpers below and is clamped to this
    // window. The bounds only ever fire at the section boundary, so parsing of
