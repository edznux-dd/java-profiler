...
// Fuzz-only thunk: ElfParser is a translation-unit-local class, so the public
// parseFile() static is not reachable from another file. This forwards a fuzz
// harness into it. base==NULL keeps symbol addresses as raw st_value (never
// dereferenced) and makes calcVirtualLoadAddress() a no-op, so the only reads
// are file-offset-relative into the mmap'd image — exactly the untrusted
// parsing surface we want to exercise.
bool Symbols::parseElfFileForFuzzing(CodeCache* cc, const char* file_name, bool use_debug) {
    return ElfParser::parseFile(cc, NULL, file_name, use_debug);
}
#endif
...
const uint8_t* SymbolsLinux::findBuildIdInNotes(const void* note_data, size_t note_size, size_t* build_id_len) {
    ...
    while (offset < note_size) {
        ...
        // Read the note header fields with memcpy: note_data is base + p_offset
        // and p_offset is attacker-controlled, so the struct may be misaligned.
        // Dereferencing an Elf64_Nhdr* there would be undefined behaviour (and a
        // real fault on alignment-strict architectures).
        uint32_t n_namesz, n_descsz, n_type;
        memcpy(&n_namesz, data + offset, sizeof(n_namesz));
        memcpy(&n_descsz, data + offset + 4, sizeof(n_descsz));
        memcpy(&n_type,   data + offset + 8, sizeof(n_type));
        ...
        size_t name_size_aligned = (static_cast<size_t>(n_namesz) + 3) & ~static_cast<size_t>(3);
        size_t desc_size_aligned = (static_cast<size_t>(n_descsz) + 3) & ~static_cast<size_t>(3);
